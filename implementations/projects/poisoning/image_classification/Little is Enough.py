...
class GradientInverter:
    ...
    def _find_best_z(self, avg_grad: Tensor, std_grad: Tensor, model_grad: Tensor) -> float:
        #We try multiple value of z and choose the best one
        candidate_z = [0.5, 1.0, 2.0, 3.0, 4.0]  # Example of values
        best_z = candidate_z[0]
        best_loss = float('-inf')

        for z in candidate_z:
            crafted_target = avg_grad + z * std_grad
            loss = F.mse_loss(model_grad, crafted_target)
    ...
    def attack(self, model: nn.Module, criterion: _Loss) -> tuple[Tensor, Tensor]:
        ...
        **WARNING**: do not deepcopy the model before calling this function
        ...
        if self.method == GradientAttack.LITTLE_IS_ENOUGH:
            avg_grad = self.estimator.average_clean_gradient(model, criterion)
            std_grad = self.estimator.std_clean_gradient(model, criterion)

            #we use a preliminary model to choose z
            preliminary_model_grad = combined_model_gradients(model)

            z = z = self._find_best_z(avg_grad, std_grad, preliminary_model_grad)

            #We calculate the final gradient of the attack (LIE)
            target_grad = avg_grad + z * std_grad
